...
export default function DevisModal({ editingDevis, initialClientId, onSave, onClose, isSaving }: Props) {
  ...
  const sendEmail = () => {
    ...
    const signatureLine = form.signature_activee
      ? `\n\nPour valider ce devis, signez-le en ligne ici :\n${window.location.origin}/devis/signature/${form.signature_token}\n`
      : ''
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4"
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl max-h-[92vh] flex flex-col overflow-hidden animate-slide-up">
        ...
        {/* ── Header ──────────────────────────────────────── */}
        <div className="shrink-0 relative overflow-hidden bg-gradient-to-br from-brand-600 to-blue-700 px-6 py-5">
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-white/70">
                  {editingDevis ? 'Mise à jour du devis' : 'Créer un devis pour un client'}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              {!editingDevis && (
                <button onClick={() => setShowIA(v => !v)}
                  className="btn-sm btn bg-white/15 hover:bg-white/25 text-white gap-1.5 border-0">
                  <Zap size={12} /> Auto
                </button>
              )}
              <button onClick={onClose} className="p-2 rounded-xl text-white/80 hover:bg-white/15 hover:text-white transition-colors">
                <X size={17} />
              </button>
            </div>
          </div>
